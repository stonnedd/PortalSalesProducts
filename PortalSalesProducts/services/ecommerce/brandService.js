...
brandService.getCatalogs = function (param, next) {
    async.parallel([
        ...
        if (!param.page)
            param.page = 1;
            param.rows = 12;    
            productModel.find({ category: param.category, brand: param.brand })
            ...
            if (err) {
                ...
                productModel.find({ category: param.category, brand: param.brand }).count(function (err, count) {
                    if (err) {
                        callback(err);
                    } else {
                        var productsWitdId = [];
        ...
        function (callback) {
        brandModel.find({ category: param.category }).select({ _id: 0, name: 1 }).sort('name').exec(function (err, brands) {
            if (err) {
                callback(err);
            }
            else {
